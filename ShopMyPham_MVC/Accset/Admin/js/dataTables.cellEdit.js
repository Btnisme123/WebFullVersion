    ...
    jQuery.fn.extend({
        ...
        updateEditableCell: function (callingElement) {
            ...
            function _update(newValue) {
                ...
                debugger;
                //Return cell & row.
                settings.onUpdate(cell, row, oldValue,row.data());
            }
    ...
    $(table.body()).on('click', 'td', function () {
        ...
        if ((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) {
            ...
            if (!$(cell).find('input').length && !$(cell).find('select').length) {
                // Input CSS
                var input = getInputHtml(currentColumnIndex, settings, cell);
...
function getInputHtml(currentColumnIndex, settings, cell) {
    ...
    switch (inputType) {
        ...
        case "undefined-confirm": // text input w/ confirm
            ...
            break;
        default: // text input
            input.html = "<input id='ejbeatycelledit' class='" + inputCss + "' onfocusout='$(this).updateEditableCell(this)' value='" + cell.textContent + "'></input>";
            break;
    }
    return input;
}
